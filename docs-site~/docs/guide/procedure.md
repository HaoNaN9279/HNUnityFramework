...
| 转换方式 | 手动调用 ChangeProcedureState | 自动条件评估 + 转换 |
...
| `IProcedureManager` / `ProcedureManager` | 单例管理器，管理状态注册和切换 |
...
ProcedureManager.AddProcedureState<GamePlayProcedure>("GamePlay");

// 启动流程
ProcedureManager.StartProcedure("GameStart");

// 每帧驱动（由 HNUnityFramework 自动调用）
...
// 通过引用切换
var gamePlay = ProcedureManager.AddProcedureState<GamePlayProcedure>("GamePlay");
ProcedureManager.ChangeProcedureState(gamePlay);
```
...
```
注册:   AddProcedureState<T>(name) → ProcedureState.Initialize(name)
        添加状态到内部字典

启动:   StartProcedure(name) → currentState.OnEnter()
        设置当前状态并触发进入事件

每帧:   Tick() → currentState.InvokeTickEvent()
        LateTick() → currentState.InvokeLateTickEvent()

切换:   ChangeProcedureState(newName)
...
```csharp
// 获取当前状态
ProcedureState current = ProcedureManager.CurrentState;

// 获取状态数量
int count = ProcedureManager.ProcedureStateCount;

// 动态移除状态
ProcedureManager.RemoveProcedureState("OldState");
```
...
- Procedure 没有 HFSM 的"条件自动转换"——所有切换需要手动调用 `ChangeProcedureState`
- Procedure 适合顶层流程管理（几个互斥的大阶段），HFSM 适合有复杂状态切换的局部逻辑
- 框架主循环 `HNUnityFramework.OnAwake()` 中自动调用 `ProcedureManager.Initialize()`
